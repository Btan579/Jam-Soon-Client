...
export const fetchEvents = (metroCode, dateSelected) => async dispatch => {
    let eventsArr = [];

    await fetch(`${SONG_KICK_BASE_URL_EVENT}${metroCode}/calendar.json?min_date=${dateSelected}&max_date=${dateSelected}&apikey=${SONG_KICK_KEY}`)
        .then(async res => {
            if (await !res.ok) {
                return Promise.reject(res.statusText);
            }
        ...
        .then( async data => {
            ...
            await dispatch(fetchEventsSuccess(eventsArr));
        });
};
