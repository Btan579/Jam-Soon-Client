...
export const fetchEvents = (metroCode, dateSelected) => dispatch => {
    ...
    fetch(`${SONG_KICK_BASE_URL_EVENT}${metroCode}/calendar.json?min_date=${dateSelected}&max_date=${dateSelected}&apikey=${SONG_KICK_KEY}`)
        ...
        .then(data => {
            ...
            dispatch(fetchYTplaylists(performingArtists));
        });
};


export const fetchYTplaylists = (performingArtists) => dispatch => {
    let videosArr = [];
    for (let j = 0; j < performingArtists.length; j++) {
        let videoQuery = performingArtists[j].artistName
        let act = performingArtists[j];
    ...
    dispatch(fetchYTSuccess(videosArr));
    
};

export const FETCH_YT_VIDEOS_SUCCESS = 'FETCH_YT_VIDEOS_SUCCESS';
export const fetchYTSuccess = (videosArr) => ({
