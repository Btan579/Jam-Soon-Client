...
export const fetchEventsSuccess = (eventDay, eventName, headliner, headlinerPlaylist, secondArtist, secondArtistPlaylist, venueName, venueLocation, event_id, performingArtists, perfArray) => ({
...
export const fetchEvents = (metroCode, dateSelected) => dispatch => {
    ...
    fetch(`${SONG_KICK_BASE_URL_EVENT}${metroCode}/calendar.json?min_date=${dateSelected}&max_date=${dateSelected}&apikey=${SONG_KICK_KEY}`)
        ...
        .then(data => {
            ...
            let ploop = pdata.resultsPage.results.event;
            for (let j = 0; j < ploop.length; j++) {
                let perfArray =[];
                let perfs = ploop[i].performance;
                perfs.forEach(function (perf) {
                    let pQuery = perf.displayName;
                    fetch(`${YOU_TUBE_BASE_URL}?part=snippet&q=${pQuery}&type=playlist&maxResults=1&key=${YOU_TUBE_KEY}`)
                        .then(rez => {
                            return rez.json();
                        })
                        .then(ndata => {
                            let akt = {
                                ...
                                artistName: perf.artist.displayName,
                                billSlot: perf.billing,
                                billIndex: perf.billingIndex,
                                // event_id: event_id,
                                playlist_id: ndata.items[0].id.playlistId,
                                thumbnail: ndata.items[0].snippet.thumbnails.high.url
                            };
                            perfArray.push(akt);
            ...
            for (let i = 0; i < events.length; i++) {
                ...
                dispatch(fetchEventsSuccess(eventDay, eventName, headliner, headlinerPlaylist, secondArtist, secondArtistPlaylist, venueName, venueLocation, event_id, performingArtists, perfArray));
            }
