...
export const fetchEventsSuccess = (eventDay, eventName, headliner, headlinerPlaylist, secondArtist, secondArtistPlaylist, performingArtists, venueName, venueLocation, event_id) => ({
    ...
    fetch(`${SONG_KICK_BASE_URL_EVENT}${metroCode}/calendar.json?min_date=${dateSelected}&max_date=${dateSelected}&apikey=${SONG_KICK_KEY}`)
        ...
        .then(data => {
            ...
            for (let i = 0; i < events.length; i++) {
                ...
                //    for (let j = 0; j < performanceArray.length; j++) {

                //     let headliner = performanceArray[j].artistName;
                ...
                // }
                    ...
                    let secondArtist = "pogu";
                    let secondArtistPlaylist = "pog.w.net";
                    let performingArtists = performanceArray;
                    let venueName = events[i].venue.displayName;
                    let venueLocation = events[i].location.city;
                    let event_id = events[i].id;

                dispatch(fetchEventsSuccess(eventDay, eventName, headliner, headlinerPlaylist, secondArtist, secondArtistPlaylist, performingArtists, venueName, venueLocation, event_id));
            }
        });
};
