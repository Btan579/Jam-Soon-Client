...
export class EventsSection extends React.Component {
    ...
    render() {
        ...
        const performers = this.props.playlistsData.filter(performer => {
            return performer.event_id === eventId;
        });
        const ham = performers.sort((a, b) => (a.billIndex > b.billIndex) ? 1 : -1);

        const perfs = performers.map((performer, index) => (
