...
describe('<FavoriteArtists />', () => {
    ...
    it('Renders favorite artist name', () => {
        ...
        jestExpect(wrapper.contains(<h4>{favArtistName}</h4>)).toEqual(true);
    });

    it('Renders fav artist hr', () => {
        const wrapper = shallow(<FavoriteArtists />);
        jestExpect(wrapper.contains(<hr className="fav-artist-hr"></hr>)).toEqual(true);
    });

    it('Renders favorite-artist container', () => {
        const wrapper = shallow(<FavoriteArtists />);
        jestExpect(wrapper.hasClass('favorite-artist')).toEqual(true);
    });

    it('Renders delete fav artist button', () => {
        const wrapper = shallow(<FavoriteArtists />);
        jestExpect(wrapper.find('button').hasClass('fav-artist-delete-btn')).toEqual(true);
    });
    ...
    it('Renders favorite artist div with data attributes', () => {
        ...
        jestExpect(wrapper.find('.favorite-artist').prop("data-user")).toBe(currentUser_id);
        jestExpect(wrapper.find('.favorite-artist').prop("data-artist_id")).toBe(artist_id);
        jestExpect(wrapper.find('.favorite-artist').prop("data_id")).toBe(_id);
    });
