...
export class EventsSection extends React.Component {
    ...
    render() {
      ...
      const perf = artists.map((artist) => 
          <div data-bill-index={artist.billIndex}>
              <h5>{artist.artistName}</h5>
              <p>{artist.artist_id}</p>
              <p>{artist.billIndex}</p>
              <p>{artist.billSlot}</p>
              <a target='_blank' href={`https://www.youtube.com/playlist?list=${artist.playlist_id}`}><img classname='vidThumbnail' alt={`${artist.artistName} playlist thumbnail`} src={artist.thumbnail} /></a>
              <button type="button" onClick={() => this.saveArtist(artist)}>Favorite artist</button>
          </div>
      ); 
        return (
            <div className="event-section"
                ...
                data-partists={this.props.performingArtists}
            >
